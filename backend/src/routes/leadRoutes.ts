...
const allowedStatuses: LeadStatus[] = ["MANDADO", "VISUALIZOU", "RESPONDEU", "NEGOCIANDO", "FECHADO"];
const allowedChannels: LeadChannel[] = ["INSTAGRAM", "WHATSAPP", "OUTRO"];
...
router.get("/", async (req, res) => {
  try {
    ...
    const normalizedStatus = allowedStatuses.includes(statusParam as LeadStatus)
      ? (statusParam as LeadStatus)
      : undefined;
    const normalizedChannel = allowedChannels.includes(channelParam as LeadChannel)
      ? (channelParam as LeadChannel)
      : undefined;
...
router.post("/", async (req, res) => {
  try {
    ...
    const normalizedChannel = channel.toString().toUpperCase() as LeadChannel;
    if (!allowedChannels.includes(normalizedChannel)) {
      return res.status(400).json({ message: "Canal inválido" });
    }

    const normalizedStatus = status
      ? (status.toString().toUpperCase() as LeadStatus)
      : undefined;
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const normalizedChannel = channel
      ? (channel.toString().toUpperCase() as LeadChannel)
      : undefined;
